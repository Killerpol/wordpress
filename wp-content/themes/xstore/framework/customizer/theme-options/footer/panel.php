<?php  
	...
	add_filter( 'et/customizer/add/panels', function($panels) use($priorities){
        ...
        if ( $elementor_templates_count > 0 ) {
            $args['footer']['description'] = sprintf(esc_html__('It looks like you\'re using Elementor %sFooter templates%s [%s] on your site. This might cause issues with your customizer footer.', 'xstore'), '<a href="'.admin_url( 'admin.php?page=et-panel-theme-builders' ).'" target="_blank">', '</a>', $elementor_templates_count);
        }
