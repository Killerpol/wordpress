...
if ( !$allow_full_access && in_array('plugins', $show_pages) && !class_exists('XStore_White_Label_Branding') ) {
    ...
    $categories[$category]['items'][] = sprintf(
        ((!$core_active || !$theme_active || !$amp_active) && !$allow_full_access ) ? '<li class="mtips'.((!$core_active || !$theme_active) ? ' inactive' : '').'"><a href="%s" class="%s">%s %s</a><span class="mt-mes">' . $branding_tips . '</span></li>' : '<li><a href="%s" class="%s">%s %s</a></li>',
