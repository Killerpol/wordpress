...
if(!function_exists('etheme_load_admin_styles')) {
	...
	function etheme_load_admin_styles() {
		...
		if ( strpos($screen_id, 'et-panel') ) {
		    wp_enqueue_style('etheme_admin_panel_css', ETHEME_CODE_CSS.'et_admin-panel-styles.css');
		    ...
		    if ( strpos($screen_id, 'et-panel-theme-builders') ) {
		        if ( defined('ET_CORE_VERSION'))
		            wp_enqueue_style('et-core-eight_theme-elementor-icon', ET_CORE_URL . 'app/assets/css/eight_theme-elementor-icon.css');

		        wp_enqueue_style('etheme_admin_panel_theme_builders_css', ETHEME_CODE_CSS.'et_admin-panel-theme-builders.css');
		    }
            ...
            if ( $screen_id == 'elementor_page_elementor-element-manager' ) {
                if ( defined('ET_CORE_VERSION'))
		            wp_enqueue_style('et-core-eight_theme-elementor-icon', ET_CORE_URL . 'app/assets/css/eight_theme-elementor-icon.css');
            }
        }
...
//	    wp_enqueue_script('et_masonry', get_template_directory_uri().'/js/jquery.masonry.min.js',array(),false,true);
    	...
    	wp_localize_script( 'etheme_admin_js', 'et_variation_gallery_admin', $localize_admin_script );
	}
}
...
function et_add_product_bought_together_panel_tab() {
        ...
        <li class="et_bought_together_options et_bought_together_tab show_if_simple show_if_external">
            ...
            <?php echo '<span class="et-brand-label" style="background: var(--et_admin_dark-color, #222); color: #fff; font-size: 0.65em; line-height: 1; padding: 2px 5px; border-radius: 3px; margin: 0; margin-inline-start: 3px;">'.$label.'</span>'; ?>
            </span></a>
        </li>
        <?php
    }
...
function et_ai_msg(){
    ...
    if (class_exists( 'WooCommerce' )){
        ...
        $woo_msg = '
            <p>'.sprintf(esc_html__('Additionally, you can generate content for your, products by going to %s WooCommerce -> Products -> Edit %s and scrolling down to the bottom of the page.', 'xstore'), '<a href="'. $url .'" target="_blank">', '</a>') . '</p>
        ';
    }
    ...
    return '
        <div class="et_ai-saved-options">
            ...
            '<a href="'. $url .'" target="_blank">', '</a>', $brand_label).'</p>'.$woo_msg.'
        </div>
    ';
}
