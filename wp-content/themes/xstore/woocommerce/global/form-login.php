...
<form method="post" class="woocommerce-form woocommerce-form-login login" <?php if ( $hidden ) echo 'style="display:none;"'; ?>>
	...
	<p class="woocommerce-form-row woocommerce-form-row--wide form-row form-row-wide">
		...
		<input type="text" class="input-text" name="username" id="username"<?php if ($layout_new) : ?> value="<?php echo ( ! empty( $_POST['username'] ) ) ? esc_attr( wp_unslash( $_POST['username'] ) ) : ''; ?>"<?php endif; ?>/>
	</p>
	<p class="woocommerce-form-row woocommerce-form-row--wide form-row form-row-wide">
		<label for="password"><?php esc_html_e( 'Password', 'xstore' ); ?> <span class="required">*</span></label>
		<input class="input-text woocommerce-Input" type="password" name="password" id="password" <?php if ($layout_new) : ?> autocomplete="current-password"<?php endif; ?>/>
	</p>
	<div class="clear"></div>
