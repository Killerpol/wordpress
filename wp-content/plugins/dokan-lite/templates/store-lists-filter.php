...
<div id="dokan-store-listing-filter-wrap">
    ...
    <div class="left">
        <p class="item store-count">
            ...
            printf( esc_html( _n( 'Total de tienda mostrada: %s', 'Total de tiendas mostradas: %s', $number_of_store, 'dokan-lite' ) ), esc_html( number_format_i18n( $number_of_store ) ) );
            ?>
        </p>
    </div>
...
<form role="store-list-filter" method="get" name="dokan_store_lists_filter_form" id="dokan-store-listing-filter-form-wrap" style="display: none">
    ...
    if ( apply_filters( 'dokan_load_store_lists_filter_search_bar', true ) ) :
        ...
        <div class="store-search grid-item">
            <input type="search" class="store-search-input" name="dokan_seller_search" placeholder="<?php esc_attr_e( 'Buscar Vendedores', 'dokan-lite' ); ?>">
        </div>
        <?php
    endif;
