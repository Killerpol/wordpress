...
//        }
    ...
    public function menu_cart_fragments() {
        ...
        if ( ! isset( $_POST['_nonce'] ) || ! wp_verify_nonce( wp_unslash( $_POST['_nonce'] ), str_replace(array('cart', 'wishlist', 'compare'), array(self::MENU_CART_FRAGMENTS_ACTION, self::MENU_WISHLIST_FRAGMENTS_ACTION, self::MENU_COMPARE_FRAGMENTS_ACTION), $_POST['nonce_type']) ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, it's used only for nonce verification
//            write_log('nonce not validated');
            wp_send_json( [] );
        }
//        write_log('nonce validated');
...
//        write_log('menu_cart_fragments 222');
...
//        write_log('menu_cart_fragments 333');
    ...
    public function get_location_widgets_assets($location = 'header') {
        ...
        foreach ($documents as $document) {
            if ( is_object( $document ) ) {
                ...
                if ( count(array_intersect(array('theme-etheme_cart', 'theme-etheme_wishlist', 'theme-etheme_compare'), $widgets_2_load)) ) {
