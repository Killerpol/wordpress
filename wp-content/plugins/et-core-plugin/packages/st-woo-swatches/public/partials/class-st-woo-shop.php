...
//		add_action( 'woocommerce_shop_loop_item_title', 'woocommerce_template_loop_product_link_open', 9 );
    ...
    public function loop_swatch( $size = 'normal', $is_quick_view = false ) {
        ...
        foreach( array_keys( $attributes ) as $taxonomy ) {
            ...
            if( array_key_exists( $attribute->attribute_type,  $attributes_types_list ) ) {
                ...
                if ($variations_images){
                    if ( in_array($attribute->attribute_type, array('st-image-swatch', 'st-image-swatch-sq')) ) {
                        $variations_by_attr = $this->variations_by_attr($available_options, $key, $variations);
                    }
                }
                if ($show_select_swatch && $attribute->attribute_type == 'select') {
                    $variations_by_attr = $this->variations_by_attr($available_options, $key, $variations);
                }
                if ( !isset($attributes_count[$attribute->attribute_type]) )
                    $attributes_count[$attribute->attribute_type] = 0;
                $attributes_count[$attribute->attribute_type]++;

                $html .= apply_filters( 'sten_wc_archive_loop_swatch_html', $attribute->attribute_type, $taxonomy, $terms, $available_options, $variations, $selected, $attributes_count, $size, $is_quick_view, $variations_by_attr );
            }
        }
